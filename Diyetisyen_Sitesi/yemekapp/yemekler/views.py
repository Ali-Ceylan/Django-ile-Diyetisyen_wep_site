...
def mail_gonder(request):
    ...
    email_icerik = f"""
        Diyetisyeninizden size günlük yemek programı geldi.
        <table border="1">
            <tr>
    ...
    if request.method == 'POST':
        ...
        konu = my_konu,request.user.first_name,request.user.last_name
        icerik = email_icerik
        ...
        try:
            obj.save()
            send_mail(
            from_email='diyetisyendiyet190@gmail',  # Gönderen e-posta adresi
            subject=konu,
            message='',
...
def randevular(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = f'Randevu Talebi isim ',request.user.first_name ,request.user.last_name
